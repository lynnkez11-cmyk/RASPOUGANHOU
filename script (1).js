...
function scratch(e) {
    ...
    // Draw scratch mark with varying size
    ctx.beginPath();
    ...
    for (let i = 0; i < 3; i++) {
        ...
        ctx.beginPath();
        ctx.arc(x + offsetX, y + offsetY, 8, 0, 2 * Math.PI);
        ctx.fill();
    }
...
function updateScratchedPercentage() {
    ...
    // Check if enough area is scratched (10% threshold)
    if (gameState.scratchedPercentage >= 10 && gameState.isGameActive) {
        revealPrize();
    }
}
